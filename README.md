...
2. **Запуск утиліти**:
   ...
   - Звичайний режим:
     ...
     ```bash
     java -jar target/cip_gov_ua_getter-3.4.0-all.jar
     ```

   - Дебаг-режим:

     ```bash
     java -jar target/cip_gov_ua_getter-3.4.0-all.jar -d
     ```
...
- **Дорадчий список кандидатів (`possible_missed_domains.txt`)**: після основного розбору кожного PDF утиліта ще раз проходить той самий текст **іншою** стратегією об'єднання рядків (перенос → пробіл, замість видалення) і виписує в цей файл лише те, чого немає в основному результаті. Це **пропозиції для людини**, а не блокування: у `blocked.result.txt` вони не потрапляють ніколи. Детальніше — у FAQ нижче.
...
Це **дорадчий** список — те, що знайшов додатковий (не авторитетний) прохід по тексту PDF і чого немає в основному результаті. У `blocked.result.txt` ці домени не потрапляють ніколи: файл існує лише для того, щоб людина оком глянула, чи не ховається там щось справжнє. Записи не дублюються — файл читається, доповнюється й переписується атомарно, тож повторні запуски нічого не множать. Частина пропозицій свідомо буде «сміттям» (наприклад, `new.com` — уламок домену, перенесеного PDF-редактором посеред імені): це плата за те, що прохід дивиться на текст під іншим кутом.
...
new.com
# перевірено вручну 2026-09-04 — це не домен
kinozapasho20.kinoza.to
```

Домен звідси більше не потрапить у пропозиції, навіть якщо дорадчий прохід «знайде» його знову. Сам `possible_missed_domains.txt` після цього варто видалити — він перезбереться вже без заглушених записів.

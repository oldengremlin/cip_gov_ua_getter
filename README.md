...
- **Логіка роботи**:
  ...
  - `AggressorServicesParser`: Парсить PDF із сервісами держави-агресора — сайт НЦУ (`webportal.nrada.gov.ua`). Кеш PDF оновлюється за віком (`AggressorServices_max_age_hours`), а не назавжди — на відміну від `GetPrescript` і `PlaycityParser`, де кожен документ незмінний.

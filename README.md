...
2. **Запуск утиліти**:
   ...
   - Звичайний режим:
     ...
     ```bash
     java -jar target/cip_gov_ua_getter-3.4.1-all.jar
     ```

   - Дебаг-режим:

     ```bash
     java -jar target/cip_gov_ua_getter-3.4.1-all.jar -d
     ```
...
- **Частина записів у PDF лишається `Invalid IDN domain` — і це нормально.** На «Переліку» НЦУ станом на 20.08.2026 (56 сторінок, 931 валідний домен) таких записів 42, з них:
...
На «Переліку» від 20.08.2026, звіреному з реальним переліком блокування (54 608 доменів), дорадчий прохід дав 11 кандидатів, з яких після відсіву лишилося 6 — половина корисних, половина уламків:

- **Корисні** — `ivi.ru`, `ivi.az`, `ott-antifriz.com`. Точного збігу немає, бо в блокуванні лежать лише `www.ivi.ru`, `www.ivi.az`, `www.ott-antifriz.com`: евристика зрізання склеєної схеми повертає `www.`-варіант (на момент зрізання субдомену TLD ще зіпсований, тож `www` не прибирається). Дорадчий прохід показує батьківський домен — заблокувати його означає закрити й усі субдомени.
- **Уламки** — `new.com`, `kinozapasho20.kinoza.to`, `wwwlordfilm52.kinozi.li`. Усі три — той самий дефект: PDF-редактор переніс рядок посеред імені, а склейка через пробіл лишила початок, який випадково валідний сам по собі. `https://hdplayer.kinogo-` + `new.com` дає неіснуючий `new.com`; `…kinoza.top` і `…kinozi.link`, розірвані посеред TLD, дають `.to` (Тонга) і `.li` (Ліхтенштейн) — теж валідні TLD. Ознака впізнавання: у блокуванні вже є довший домен, для якого кандидат — обрізаний початок. Відсіюються через `.ignore` за хвилину.
...
new.com
# уламки від TLD, розірваних посеред слова: .link → .li, .top → .to
wwwlordfilm52.kinozi.li
kinozapasho20.kinoza.to
```

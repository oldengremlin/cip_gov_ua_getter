...
- **Валідатор доменів**: Перевірка через `commons-validator`, обробка гомогліфів із `icu4j`. Публічні суфікси (`com.ua`, `kiev.ua`, `co.uk`) ніколи не потрапляють у результат — заблокувати такий запис означало б покласти цілу зону. Службові префікси (`www`, `ftp`, `mail`, …) зрізаються незалежно від того, яким шляхом домен потрапив у перелік — зокрема й із вхідних файлів `blocked`.
...
flowchart TD
    ...
    Store --> Missed["storePossiblyMissedDomains()<br/>дорадчі кандидати мінус усе відоме<br/>→ possible_missed_domains.txt"]
    Missed --> End(["Кінець"])
```
...
classDiagram
    ...
    class AbstractPDFParser {
        ...
        +storePossiblyMissedDomains(known)$ void
    }
    ...
    class BlockedObjects {
        ...
        +getKnownDomainNames() TreeSet~String~
        +storeState() BlockedObjects
    }

    note for BlockedObjects "addBlockedDomainName — єдина точка входу в сховище:<br/>зрізає службовий префікс і відмовляє публічному суфіксу,<br/>незалежно від джерела домену"
...
2. **Запуск утиліти**:
   ...
   - Звичайний режим:
     ...
     ```bash
     java -jar target/cip_gov_ua_getter-3.4.3-all.jar
     ```

   - Дебаг-режим:

     ```bash
     java -jar target/cip_gov_ua_getter-3.4.3-all.jar -d
     ```
...
- **Очищення**: Видаляються протоколи (`https://`, `ftp://`), службові субдомени (`www`, `mail` тощо — перелік у `SERVICE_SUBDOMAINS`), шляхи (`/path`), порти (`:8080`) та параметри (`?key=value`). Субдомен зрізається лише тоді, коли після цього лишається реєстрований домен: `www.com.ua` так і лишиться, бо `com.ua` — публічний суфікс. Зрізання застосовується ще раз після евристичного відновлення склеєного імені — інакше `www.ivi.ruhtpps` дало б `www.ivi.ru` замість зони `ivi.ru`, бо в момент першої спроби TLD ще зіпсований і перевірка на публічний суфікс не бачить реєстрованого домену.
...
- **Евристичне відновлення склеєних доменів**: `prepareDocument` прибирає переноси рядків у тексті PDF, тож кінець одного домену інколи злипається з сусіднім текстом. Обидві евристики спрацьовують **лише як фолбек** після невдачі звичайної валідації й обов'язково перевалідовують відновлений кандидат тим самим повним ланцюжком перевірок (публічний суфікс → IDN → TLD), тому гірше, ніж «нічого не робити», не буде ніколи:
  - **склеювання зі схемою наступного посилання** (`hd.muvee.me` + `https://...` → `hd.muvee.mehttps`): якщо хвіст — це `http`, `https`, `ftp` або `htpps` (підтверджений варіант `https` з переставленими літерами — артефакт шрифту в PDF), він прибирається;
  - **склеювання з номером наступного рядка таблиці** (`kinogo.ga` + `13.` + `Kinokrad` → `kinogo.ga13.kinokrad`): перебираються всі позиції `\d+.` усередині імені та береться найдовший префікс, що самостійно пройде валідацію (номер на самому початку — `77.muvee` — не рахується: це не склеювання, а сам номер рядка).

  Обидві пишуть однаковий за формою рядок логу, тож усі відновлення легко зібрати одним `grep`:

  ```bash
  grep "heuristically recovered from" logs/cip_gov_ua_getter.log
  ```

  Справді биті фрагменти без приліпленого хвоста (`77.muvee`) лишаються позначені як `Invalid IDN domain`, як і мають.

- **Дорадчий список кандидатів (`possible_missed_domains.txt`)**: під час розбору кожного PDF утиліта ще раз проходить той самий текст **іншою** стратегією об'єднання рядків (перенос → пробіл, замість видалення). Наприкінці запуску, коли відомі домени зібрані з **усіх** джерел, зі знайденого віднімається все вже відоме — і в файл потрапляє лише справді нове. Це **пропозиції для людини**, а не блокування: у `blocked.result.txt` вони не потрапляють ніколи. Детальніше — у FAQ нижче.
...
- **Частина записів у PDF лишається `Invalid IDN domain` — і це нормально.** На «Переліку» НЦУ станом на 20.08.2026 (56 сторінок, 924 валідні домени) таких записів 42, з них:
  - **40 — шум заголовків нумерованого списку**: `9.baskino`, `16.kionhttps`, `31.показzpokazz` — це номер рядка таблиці, зліплений із назвою сервісу, а не домен. Справжні домени тих самих рядків утиліта бере окремо, тож нічого не втрачається; `Invalid` тут — правильний результат, і «виправляти» його не треба;
  - **1 — злипання з фрагментом URL рішення**: `kinogo.animedir.netnatsionalnoyi` (хвіст — транслітерований шматок адреси рішення). Вузької евристики для цього немає: слаги — довільні транслітеровані українські фрази. Такі випадки натомість спливають у `possible_missed_domains.txt` (тут — як `kinogo.animedir.net`);
  - **1 — уламок назви сервісу з сусідньої колонки**: `mиpфильм.нe`. У таблиці «Переліку» ліва колонка містить назву (`MIRFILM.NET (МирФильм.нет)` з логотипом), права — власне перелік адрес. Це уламок кириличної назви з лівої колонки — з мішаниною латиниці й кирилиці від вбудованого шрифту та обрізаним TLD. Домени цього сервісу беруться з правої колонки повністю (`mirfilm.net`, `go.`, `see.`, `sek.`, `set.` та ще п'ять субдоменів з інших рядків), тож `Invalid` тут — знову правильний результат: ловити назви з колонки назв утиліта не мусить.

- **Склеювання, яке не псує валідності, лишається в результаті — і це свідоме рішення, не недогляд.** Евристики бачать склеєне ім'я лише тоді, коли воно завалило валідацію. Коли ж номер рішення чи сусідній текст злипається з доменом і дає синтаксично коректний субдомен (`53-06-1253.mirfilm.net`), воно мовчки проходить у `blocked.result.txt`. Ціна цього — зайвий субдомен зони, яка й так блокується цілком: він нічого не додає й нічого не ламає. Ціна «виправлення» — евристика, що на власний розсуд відкидає валідні субдомени, а серед них повно законних цілей блокування (`hd.baskino.website`, `go.mirfilm.net`). Тому лишаємо як є: менше зло обране навмисно. **Не «оптимізувати».**
...
**Що таке `possible_missed_domains.txt` і чи треба щось із ним робити?**  
Це **дорадчий** список — те, що знайшов додатковий (не авторитетний) прохід по тексту PDF і чого **немає серед уже відомих доменів**: ані в результаті, ані у вхідних файлах `blocked`, ані серед розблокованих. У `blocked.result.txt` ці домени не потрапляють ніколи — файл існує лише для того, щоб людина оком глянула, чи не ховається там щось справжнє.

Файл перезаписується цілком наприкінці кожного запуску, тож він завжди показує поточний стан: дублів не буває, а пропозиція зникає сама, щойно домен потрапляє в блокування. Якщо пропонувати нема чого — файл видаляється. Якщо ж жодне PDF-джерело не відпрацювало (мережа, сервер недоступний), файл не чіпається взагалі: порожній результат тоді означає «нема чого сказати», а не «пропозицій більше немає».

Уламки відсіюються автоматично. Дорадчий прохід склеює рядки пробілом, тож коли PDF-редактор переніс рядок посеред імені, лишається початок, який випадково валідний сам по собі: `kinozapasho20.kinoza.top` → `…kinoza.to` (Тонга), `wwwlordfilm52.kinozi.link` → `…kinozi.li` (Ліхтенштейн). Ознака надійна — серед відомих доменів є довший, для якого кандидат є початком, причому обрив припадає **всередину** лейбла. Такі кандидати прибираються з поясненням у лозі:

```
INFO ... Skipping possibly missed domain kinozapasho20.kinoza.to: looks like a truncated form of already known kinozapasho20.kinoza.top
```

Якщо ж одразу за кандидатом стоїть крапка, це не обрив, а звичайна ієрархія (`ivi.ru` для `ivi.ru.example`) — такий кандидат лишається. Теоретично можливий і хибний відсів: `example.co` — самостійний домен, який виглядає як обрізаний `example.com`. Ціна помилки низька — у гіршому разі людині не запропонують подивитися на ім'я, у блокування воно однаково не потрапляє, — і кожен відсів видно в лозі.

На «Переліку» від 20.08.2026, звіреному з реальним переліком блокування (54 608 доменів), дорадчий прохід дав 11 кандидатів, після відсіву лишився **один** — `new.com`. Решта розійшлася так: п'ять збіглися з уже відомими доменами; три (`ivi.ru`, `ivi.az`, `ott-antifriz.com`) перестали бути «знахідками», щойно службовий префікс почав зрізатися й після евристичного лікування — тепер вони потрапляють у результат безпосередньо; два прибрав автоматичний відсів уламків.

**Як сказати «оце більше не пропонуй»?**  
Створіть поруч файл `possible_missed_domains.ignore` — по одному домену на рядок, рядки з `#` на початку та порожні ігноруються:

```
# уламок від перенесення рядка всередині hdplayer.kinogo-new.com
new.com
```

Домен звідси більше не потрапить у пропозиції, навіть якщо дорадчий прохід «знайде» його знову. Видаляти `possible_missed_domains.txt` вручну не треба — наступний запуск перезапише його вже без заглушених записів.
